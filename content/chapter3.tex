...
\section{Introduction: The Art of Approximation}
...
\end{equation}
Here, the coefficient $x_k$ represents the $k$-th derivative of the trajectory, $x_k = \frac{d^k x}{d u^k}\big|_{u=0}$. Our objective is to determine these unknown coefficients $x_k$ sequentially.
...
To solve \eqref{eq:formal_f_equations}, we need an explicit formula for the coefficients $F_k$ in terms of our unknowns $x_k$. Calculating high-order derivatives of the composite function $F(x(u), u)$ requires the General Chain Rule. This is exactly the problem we solved in Chapter 2 using trees and partitions.

Using the \textbf{Bivariate Faà di Bruno's formula}, we can expand the total derivative. The formula accounts for the direct dependence on $u$ and the indirect dependence through $x(u)$. Evaluating this at $u=0$, we can express $F_k$ using \textbf{Bell polynomials} ($B_{n,k}$), which compactly handle the combinatorial partitions $\pi$.
...
\begin{itemize}
    ...
    \item $B_{n,k}$ are the partial Bell polynomials, depending on $x_1, \ldots, x_{n-k+1}$.
\end{itemize}

\subsection{Recursive Solution for \texorpdfstring{$x_k$}{xk}}
...
\begin{result}{The Perturbation Solution}{formal_x_solution}
    \begin{equation}
...
\begin{result}{The Perturbation Equation}{ltv_eqn}
    \begin{equation}
        \label{eq:ltv_eqn}
...
\begin{enumerate}
    \item \textbf{Universality of the Operator:} The Left-Hand Side (LHS) of \eqref{eq:ltv_eqn} depends \textit{only} on the zeroth-order solution $x_0(t)$. The differential operator is the same for $x_1, x_2, \dots, x_k$.
...
\begin{curiosity}{The Perturbation Loop}{perturbation_loop}
    \centering
    \begin{tikzpicture}[
            node distance=1.5cm,
            every node/.style={font=\small},
            pstep/.style={rectangle, draw=black!60, thick, rounded corners, minimum width=2.5cm, align=center, fill=white},
            source/.style={rectangle, draw=colorCuriosity, thick, dashed, rounded corners, minimum width=2.5cm, align=center, fill=colorCuriosity!5},
            arrow/.style={->, >=stealth, thick, color=black!70}
        ]

        % Nodes
        \node[pstep] (x0) {Zeroth Order\\$x_0(t)$};
        \node[source, right=1cm of x0] (S1) {Source $\mathcal{S}_1$\\ (Depends on $x_0$)};
        \node[pstep, right=1cm of S1] (x1) {1st Order\\$x_1(t)$};
        \node[source, below=1.2cm of S1] (S2) {Source $\mathcal{S}_2$\\ (Depends on $x_0, x_1$)};
        \node[pstep, right=1cm of S2] (x2) {2nd Order\\$x_2(t)$};

        % Arrows
        \draw[arrow] (x0) -- (S1);
        \draw[arrow] (S1) -- node[above, font=\footnotesize] {LTV Solver} (x1);
        \draw[arrow] (x0) |- (S2);
        \draw[arrow] (x1) |- (S2);
        \draw[arrow] (S2) -- node[above, font=\footnotesize] {LTV Solver} (x2);

    \end{tikzpicture}
    Each correction $x_k$ is fed into the source term $\mathcal{S}_{k+1}$ for the next iteration. The "LTV Solver" (the Green's function) remains unchanged throughout the entire process.
\end{curiosity}
